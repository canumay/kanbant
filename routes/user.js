...
router.post('/projects', isLoggedIn, async (req, res, next) => {
    try {
        ...
        let user = User.findById(req.user._id, { projects: 1 }); // Check user exists (not necessary because middleware already checked but its more safer.)
...
router.get('/columns', isLoggedIn, async (req, res, next) => {
    try {
        if (req.query.column_id) {
            ...
            res.status(200).json({ status: true, results: columns });
        }
    } catch (err) {
        res.status(500).json({ status: false, message: err }); // If any error occurs
    }
